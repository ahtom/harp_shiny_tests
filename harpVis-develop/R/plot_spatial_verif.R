...
plot_spatial_verif <- function(
  ...
  if (!is.object(verif_data) && is.character(verif_data)) {
    if (grepl(".sqlite", verif_data)) {
      library(RSQLite)
      message(verif_data)
      sql_object <- harpVis:::dbopen(verif_data)
  ...
  if (show_info) {
      ...
      print(verif_data)
      message("================================")
      message("Score name: ", score, " ", score_name) #score_name broken
  ...
  my_plot_func <- spatial_plot_func(score) #score_name breaks from shiny for some reason
  gg <- do.call(my_plot_func, c(list(plot_data, score), plot_opts))
