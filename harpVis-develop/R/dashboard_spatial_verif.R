#' @rdname dashboard_spatial_verif
#' @export

dashboard_spatial_verifUI <- function(id) {
...
#' @export
dashboard_spatial_verif <- function(
  input,
  output,
  session,
  verif_data,
  ...
  shiny::observeEvent(list(new_data(), colour_table(), time_axis()), {
    ...
    for (i in seq_along(summary_scores())) {
      ...
      dashboard_plots[[paste0("summary_", i)]] <- harpVis::plot_spatial_verif(
        verif_data(),
  ...
  shiny::observeEvent(
    ...
    for (i in seq_along(thresh_scores())) {
      ...
      if (nrow(plot_data[[thresh_table()]]) < 1) {
        ...
        dashboard_plots[[paste0("thresh_", i)]] <- harpVis::plot_spatial_verif(
          plot_data,
