...
#' @rdname options_bar_spatial
#'
#' @export
options_bar_spatialUI <- function(id) {
  ...
  shiny::tagList(
    shiny::fluidRow(id = "options_bar_spatial",
...
options_bar_spatial <- function(input, output, session) {
 ...
 data_files <- shiny::reactiveValues(
   ...
   models     = "ar13" #"Waiting for valid directory"
 )


  score_name <- "SAL"
  sql_object <- harpVis:::dbopen(input$data_dir)
  verif_data <- as.data.frame(harpVis:::dbquery(sql_object, paste("SELECT * FROM ", score_name)))
  harpVis:::dbclose(sql_object)

  return(verif_data)
}

options_bar_spatial_old <- function(input, output, session) {
 ...
 shiny::observeEvent(list(data_dir()), {
   ...
   data_files$filenames  <- dir(
     data_dir(), pattern = "harpSpatialVerif*[[:graph:]]*.rds"
   )
